...
%>
 ...
 <li><a href="#" class="active"><span class="glyphicon glyphicon-pencil"></span> Endorsement<% if(totalIndorseShifters+totalIndorseTransfers>0){ %> <span class="notification alert-notif">!</span> <% } %> </a>
  <ul class="submenu">
        <li><a href="OsgEndorse_Shifter.jsp" class="active"><span class="glyphicon glyphicon-cloud-upload"></span> Shifters <% if(totalIndorseShifters>0){ %> <span class="notification"><% if(totalIndorseShifters<=99){ %> <%= totalIndorseShifters %> <% }else{ %> 99+ <%} %></span> <% } %> </a></li>
  ...
  <fieldset>
      ...
      <table class="table table-striped table-sortable">
       ...
       while(rs.next()) {
        ...
        <td><button href="#<%=rs.getString("registrar_id")%>" class="fancybox btn">View Registrar's Memo</button><a href='dito mo lalagay yung link, yung target para magoopen sya ng new tab' target='_blank' download='Dito mo lalagay yung filename, may ganyan sa dulo kasi pdf file dapat diba.pdf'><button class = "fancybox btn" >Download</button></a></td>
            <td>
        ...
        <div id="<%=rs.getString("shifter_id") %>" style="width:600px;display: none;">
	      ...
	      <div id="<%=rs.getString("registrar_id") %>" style="width:600px;display: none;">
	        <%
